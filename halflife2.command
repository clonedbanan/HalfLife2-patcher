...
echo "--- Starting Half-Life 2 Build Setup for $MAC_ARCH_NAME ---"
...
if [[ "$RAW_LANG" == "pt-BR" || "$RAW_LANG" == "pt_BR" ]]; then
    ...
    case "$SYS_LANG" in
        "ru") VALVE_LANG_NAME="russian"; LANG_DEPOT="225" ;;
        "es") VALVE_LANG_NAME="spanish"; LANG_DEPOT="226" ;;
        "fr") VALVE_LANG_NAME="french"; LANG_DEPOT="227" ;;
        "de") VALVE_LANG_NAME="german"; LANG_DEPOT="228" ;;
        ...
        "it") VALVE_LANG_NAME="italian"; LANG_DEPOT="232" ;;
        "ja") VALVE_LANG_NAME="japanese" ;;
        "ko") VALVE_LANG_NAME="korean"; LANG_DEPOT="229" ;;
...
depotdownloader -app 220 -depot 221 -branch steam_legacy -dir "$LOCAL_DEPOTS_DIR" -username "$STEAM_USER" -password "$STEAM_PASS"

if [ -n "$LANG_DEPOT" ]; then
    echo "Running DepotDownloader for localized translation package (Depot $LANG_DEPOT)..."
    depotdownloader -app 220 -depot "$LANG_DEPOT" -branch steam_legacy -dir "$LOCAL_DEPOTS_DIR" -username "$STEAM_USER" -password "$STEAM_PASS"
fi
...
APP_BUNDLE="$TEMP_WORKSPACE/Half-Life 2.app"
...
# Create the internal boot script with a SourceMods chooser
echo "Generating SourceMods-aware application launcher..."
cat << 'EOF' > "$MAC_OS_DIR/hl2_osx"
#!/bin/bash

LAUNCH_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESOURCE_PATH="$(cd "$LAUNCH_DIR/../Resources" && pwd)"
MODS_ROOT="$HOME/Library/Application Support/SourceMods"
VALVE_LANG_NAME="__VALVE_LANG_NAME__"

mkdir -p "$MODS_ROOT"

# If a -game argument was supplied manually, respect it and skip the chooser.
for arg in "$@"; do
    if [ "$arg" = "-game" ]; then
        cd "$RESOURCE_PATH"
        exec "./hl2_osx" -language "$VALVE_LANG_NAME" -audiolanguage "$VALVE_LANG_NAME" "$@"
    fi
done
...
# Discover one-level-deep Source mods. A valid mod needs gameinfo.txt.
while IFS= read -r moddir; do
    if [ -f "$moddir/gameinfo.txt" ]; then
        folder_name="$(basename "$moddir")"
        display_name="$folder_name"

        # Prefer the mod's Game field for a nicer menu label when present.
        game_name="$(awk '
            BEGIN { IGNORECASE=1 }
            /^[[:space:]]*"?game"?[[:space:]]+/ {
                line=$0
                sub(/^[[:space:]]*"?game"?[[:space:]]+/, "", line)
                gsub(/^"|"[[:space:]]*$/, "", line)
                print line
                exit
            }' "$moddir/gameinfo.txt" 2>/dev/null)"

        if [ -n "$game_name" ]; then
            display_name="$game_name ($folder_name)"
        fi

        LABELS+=("$display_name")
        PATHS+=("$moddir")
    fi
done < <(find "$MODS_ROOT" -mindepth 1 -maxdepth 1 -type d -print 2>/dev/null)

LABELS+=("Open SourceMods Folder...")
PATHS+=("__OPEN_FOLDER__")

selection="$(osascript - "${LABELS[@]}" <<'APPLESCRIPT'
on run argv
    set picked to choose from list argv with title "Half-Life 2" with prompt "Choose Half-Life 2 or an installed Source mod:" default items {item 1 of argv} OK button name "Launch" cancel button name "Quit"
...
if [ "$selection" = "__CANCEL__" ] || [ -z "$selection" ]; then
    exit 0
fi
...
if [ "$selected_path" = "__OPEN_FOLDER__" ]; then
    open "$MODS_ROOT"
    exit 0
fi

if [ -z "$selected_path" ]; then
    osascript -e 'display alert "Half-Life 2" message "Could not resolve the selected game/mod." as critical'
    exit 1
fi

cd "$RESOURCE_PATH"
exec "./hl2_osx" -game "$selected_path" -language "$VALVE_LANG_NAME" -audiolanguage "$VALVE_LANG_NAME" "$@"
EOF

# Inject the detected Valve language after using a quoted heredoc above.
/usr/bin/sed -i '' "s/__VALVE_LANG_NAME__/$VALVE_LANG_NAME/g" "$MAC_OS_DIR/hl2_osx"
...
APP_BUNDLE="$SCRIPT_DIR/Half-Life 2.app"

# Create a Steam-like SourceMods folder for the launcher to scan.
SOURCEMODS_DIR="$HOME/Library/Application Support/SourceMods"
mkdir -p "$SOURCEMODS_DIR"
echo "SourceMods folder ready at: $SOURCEMODS_DIR"
